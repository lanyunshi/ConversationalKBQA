...
with open( "CONVEX/data/identifier_predicates.json", "r") as data:
	identifier_predicates = json.load(data)
# label_dict
with open( "CONVEX/data/label_dict.json", "r") as data:
	label_dict = json.load(data)
# predicate_frequencies_dict
with open( "CONVEX/data/predicate_frequencies_dict.json", "r") as data:
	predicate_frequencies_dict = json.load(data)
# entity_frequencies_dict
with open( "CONVEX/data/entity_frequencies_dict.json", "r") as data:
	entity_frequencies_dict = json.load(data)
# statements_dict
with open( "CONVEX/data/statements_dict.json", "r") as data:
	statements_dict = json.load(data)
# open the settings
with open( "CONVEX/settings.json", "r") as data:
	settings 		 	= json.load(data)
	wikidata_dump_path 	= settings['wikidata_dump_path']
with open("CONVEX/data/graphs.pkl", "r") as data:
...
def save_cached_data():
	# save the label_dict
	with open( 'CONVEX/data/label_dict.json', 'wb') as outfile:
		outfile.write(json.dumps(label_dict, separators=(',',':')).encode('utf8'))
	# save the predicate_frequencies_dict
	with open( 'CONVEX/data/predicate_frequencies_dict.json', 'wb') as outfile:
		outfile.write(json.dumps(predicate_frequencies_dict, separators=(',',':')).encode('utf8'))
	# save the entity_frequencies_dict
	with open( 'CONVEX/data/entity_frequencies_dict.json', 'wb') as outfile:
		outfile.write(json.dumps(entity_frequencies_dict, separators=(',',':')).encode('utf8'))
	# save the statements_dict
	with open( 'CONVEX/data/statements_dict.json', 'wb') as outfile:
		outfile.write(json.dumps(statements_dict, separators=(',',':')).encode('utf8'))
	# save the statements_dict
	with open( 'CONVEX/data/graphs.pkl', 'wb') as outfile:
		outfile.write(pickle.dumps(graphs))

#####################################################
###		Data fetching
#####################################################

def get_all_statements_of_predicate(predicate_id):
	# check entity pattern
	if not predicate_pattern.match(predicate_id.strip()):
		return False
	if statements_dict.get(predicate_id) != None:
		return statements_dict[predicate_id]
	predicate = "http://www.wikidata.org/predicate/"+predicate_id
	statements = []
	# predicate
	triples_sub, cardinality_sub = document.search_triples("", predicate, "")
	print('triples_sub', triples_sub)
...
def statements_to_answer(statement, path2ans, sub=None, idx=0):
	if statement['qualifiers']:
		for qualifier in statement['qualifiers']:
			path = (qualifier['qualifier_predicate']['id'], )
			if sub is not None: path = (sub, ) + path
			if path not in path2ans: path2ans[path] = [set(), 0]
			answer = qualifier['qualifier_object']['id']
			path2ans[path][0].add(answer)
	else:
		path = (statement['predicate']['id'], )
		if sub is not None: path = (sub, ) + path
		if path not in path2ans: path2ans[path] = [set(), 0]
		answer = statement['object']['id']
		path2ans[path][0].add(answer)
	path2ans[path][1] = idx
	return path2ans
...
def append_graph(q_idx, graph):
	graphs[q_idx] += [graph]

def do_exists_in_graph(q_idx, time):
	if (q_idx in graphs) and (time < len(graphs[q_idx])):
		return True
	else:
		return False

def access_graph(q_idx):
	return graphs[q_idx]
