...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states, mode='FirstPool'):
        ...
        elif mode == 'BiFirstPool':
            b, ql, h = hidden_states.size()
            hidden_states = hidden_states.view(b, ql, 2, h/2)
            #print('hidden_states', hidden_states.size())
            first_token_tensor = torch.cat([hidden_states[:, 0, 0, :], hidden_states[:, -1, 1, :]], -1)
        else:
            first_token_tensor = hidden_states[:, mode]
        #print('first_token_tensor', first_token_tensor.size())
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        #print('query_layer key_layer', query_layer[0, 0, :, 0], key_layer[0, 0, :, 0])
        ...
        #print('attention_probs', attention_probs[0, 0, 1, :]); exit()
        # print('\n')
